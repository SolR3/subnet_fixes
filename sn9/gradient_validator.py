...
class GradientValidator(BaseNeuron):
        ...
        while True:
            try:
                ...
                if await self.api_client.health_check():
                    ...
                    #     }
                    #
                    # the keys in global_weights is a mix of uids and hotkeys
                    #
                    m = self.metagraph or self.subtensor.metagraph(netuid=int(settings.netuid))
                    gw_temp = {}
                    for key, weight in global_weights.items():
                        try:
                            ...
                            uid = int(m.uids[m.hotkeys.index(key)])
                        gw_temp[uid] = weight
                    global_weights = gw_temp
                else:
                    register_validator = True
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                if success:
                    ...
                    if "No attempt made." in response:
                        break
                    if attempt < max_retries - 1:
                        logger.error(f"Retrying in {delay_between_retries} seconds...")
                        time.sleep(delay_between_retries)
                    else:
                        logger.error(f"Failed to submit weights after {max_retries} attempts.")
    ...
    def copy_weights_from_chain(self) -> dict[int, float]:
        ...
        # zero out out all the values except for the max one
        valid_stakes[valid_stakes != np.max(valid_stakes)] = 0.0
        # valid_vTrusts = meta.Tv[valid_indices]
        normalized_stakes = valid_stakes / np.sum(valid_stakes)
        # normalized_vTrusts = valid_vTrusts / np.sum(valid_vTrusts)
        stake_weighted_average = np.dot(normalized_stakes, valid_weights).astype(float).tolist()
        # stake_weighted_average = np.dot(normalized_vTrusts, valid_weights).astype(float).tolist()
