...
async def main() -> None:
    ...
    # await chain.start()  # Sol
    logger.debug(
        f"Connected to the chain netuid={chain.netuid} with coldkey '{chain.coldkey[:2]}***', "
        f"hotkey '{chain.hotkey[:2]}***'"
    )

    miner_scorer = MinerScorer(chain=chain, weight_syncer=None, **config.miner_scorer.kwargs)
    asyncio.create_task(miner_scorer.start_loop())

    logger.debug("Starting validator with full burn mode")
    try:
        while True:
