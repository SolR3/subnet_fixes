...
class MinerScorer:
    ...
    async def start_loop(self) -> None:
        ...
        while self._running:
            ...
            success = self.set_scores()
            if success:
                logger.info("Successfully set weights")
            else:
                logger.error("Failed to set weights")
            interval_minutes = self.interval // 60
            logger.info(f"Waiting {interval_minutes} minutes until next set weights attempt.")
    ...
    def set_scores(self) -> bool:
        ...
        """
        self.chain.start()  # Sol
        set_weights_result = self.chain.full_burn()
        self.chain.shutdown()  # Sol
